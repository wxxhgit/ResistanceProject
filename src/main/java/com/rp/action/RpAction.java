...
//    }
     ...
     * 方法序号： 8_10 提交 感性负载测量
     */
    public void addGxfzcl() throws Exception {
        String zsbh = this.getRequest().getParameter("zsbh");//送检仪器证书编号
        String dw = this.getRequest().getParameter("dw");//单位
        ArrayList<CaiYangGxfzcl> caiYangGxfzclList = new ArrayList<CaiYangGxfzcl>();

        for (int i = 1; i <= 3; i++) {
            CaiYangGxfzcl caiYangGxfzcl = new CaiYangGxfzcl();
            caiYangGxfzcl.setShijizhi(this.getRequest().getParameter(
                    "shijizhi" + i));
            caiYangGxfzcl.setZhishizhi(this.getRequest().getParameter(
                    "zhishizhi" + i));
            caiYangGxfzcl.setGzdl(this.getRequest().getParameter("gzdl" + i));
            caiYangGxfzcl.setZsbh(zsbh);
            caiYangGxfzcl.setDw(dw);
            caiYangGxfzcl.setId(i);

            caiYangGxfzclList.add(caiYangGxfzcl);
        }
        boolean result = rpService.addGxfzcl(caiYangGxfzclList);
        String flag;
        if (result) {
            flag = "1";
        } else {
     ...
     */
    public String deleteGxfzcl()throws Exception{
        String title = this.getRequest().getParameter("zsbh");// 证书编号
        boolean result = rpService.deleteGxfzcl(title);
        String flag;
        if (result) {
            flag = "删除成功！";
        } else {
     ...
     */
    public String findGxfzcl() throws IOException {
        String zsh = this.getRequest().getParameter("id");// 证书编号
        String result = rpService.findGxfzcl(zsh);// 打包json数据
        JSONArray allJsonArray = new JSONArray(result);
        JSONObject jo = new JSONObject();
        jo.put("allJsonArray", allJsonArray);
        this.getResponse().setContentType("text/html;charset=UTF-8");// 设置响应数据类型
